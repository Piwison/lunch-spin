...
- ⏸ **6c 結果畫面上半部很空**：是設計（鏡頭推進 + 結果出現時輪盤後退、blur 7px、0.72），app 和 `/w/:id` 一樣。
  ...
  A 維持、B 首頁不後退（看得到指針停在得獎格）、C 轉之前先捲動讓輪盤佔滿畫面。
  ✅ **業主選 B**（2026-09-23）：只有首頁 `receded={false}`，app 和 `/w/:id` 維持原設計。實測 390×844 淺／深色、
  375×667 結果都正確；小手機上結果面板本身就佔大半螢幕，上方露出的是標題和指針
...
## 7. 2026-09-24 地毯式走查（staging 同版本，本機複本）

完整報告：`docs/user-tests/2026-09-24-walkthrough.md`（含截圖、證據等級、排除的假警報）。
一句話：轉完之後的事 app 都不知道 — 不記得今天決定了什麼，也不知道最後吃了哪家，
所以紀錄、不重複、統計、團隊功能都建立在「轉了幾次」上。

| # | 項目 | 證據等級 | 需要業主決定？ |
|---|---|---|---|
| 7a | **「去吃了」變預設**：按 Directions、或沒重轉就關掉結果 = 接受；決定前的「跳過 3 天」文案改掉（P0-B） | 已驗證 | **是**：自動接受規則 |
| 7b | **今日卡片**：鎖定後輪盤頁主角換成今天的店；團隊當天已有決定時再轉要確認（P0-A） | 已驗證 | **是**：一天一個決定或多個 |
| 7c | 「Use my location」建立的輪盤沒有步行時間 → 建立後問「存成辦公室？」（P0-C） | 已驗證 | **是**（隱私文案） |
| 7d | 隊友收到的 toast 比轉的人先揭曉結果；首頁「everyone watches the same result land」不成立（P0-D） | 已驗證 | **是**：延後／重播／改文案 |
| 7e | 評分從結果卡移到下次打開（「昨天去了嗎？★」），並說明評分會影響機率（P1-I） | 已驗證 + 已確認 | 否 |
| 7f | 紀錄頁改午餐日記（一天一列、日期、重轉附在下面），刪掉 Total spins / Favourite / Most picked / Who's been picking；Places tried 只算真的去過（第 5 節） | 已驗證 | 否（依賴 7a、7e） |
| 7g | 首次設定搜尋會取代清單（7→5）；首頁草稿和 Google 結果不合併（同店兩格）；「Spin these 8」不會轉（P1-E/F/G） | 已驗證 | 否 |
| 7h | Places 語言跟瀏覽器語言而不是介面語言（預設英文的連帶影響，P1-H） | 已驗證 | 否 |
| 7i | **bug**：複製輪盤不重算步行時間，全部變 no location（`copyWheelRestaurants` 註解說呼叫端會重算，實際沒有）（P1-L） | 已驗證 | 否 |
| 7j | 快關門的店仍可被抽中（剩餘營業 < 步行時間）（P1-M） | 已確認 | 否 |
| 7k | 「Veto — not today」「Avoid today」沒有過期，只有 Clear round 會清（且會清掉別人的）；料理 chip 對沒標籤的 Google 店無效；投票／否決 30×22、只有 title（P1-K/K2） | 已驗證 + 已確認 | **是**：飲食偏好改成個人長期設定 |
| 7l | 店家詳情補 Google 評分、價位、營業到幾點、上次吃；店家頁 Nearby 改用首次設定的挑選清單（P1-N/O） | 已驗證 | 否 |
| 7m | 擁有者刪帳號會刪掉團隊輪盤，沒有警告也不能轉移（P1-P） | 已驗證 + 已確認 | 否 |
| 7n | P2 小項：switch 的 label 沒連上且只有 18px、設定一打開就全選名稱、starter pack 假店名、1 家也能轉、文案（`Hid 1 places`、`located/skipped`、`sidebar`、`Now`）、邀請對話框沒有分享、桌面側欄名稱截斷、`daysSinceLastPick` 沒擋負數、本機 http 無法登入登出（`SameSite=None` 無 `Secure`） | 已驗證 | 否 |

建議順序：7a → 7b → 7c → 7e → 7f（7f 要等資料正確才有意義），bug 7i/7j 隨時可以插隊。
...
- 2026-09-23 — 第 6 節：staging 回饋。底部面板標題列、輪盤字置中完成；結果畫面上半部業主選 B（首頁不後退）。
- 2026-09-24 — 第 7 節：staging 同版本的地毯式走查（本機複本：真程式碼 + MariaDB + 假 Google + 三週歷史）。
