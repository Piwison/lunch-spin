...
**實作計畫：`docs/plans/2026-09-24-walkthrough-plan.md`**（2026-09-24 定稿）。每一項都已經拆成工作包：
- P0：schema，兩條線共用，先做
- A 線：規則、今天、紀錄
- B 線：首次設定、店家、設定

每個工作包寫了檔案、先寫的測試、驗收步驟。驗收可以在 `scripts/replica/`（本機複本）上跑，但最後以 staging 為準。

| # | 項目 | 證據等級 | 業主決定 | 工作包 | 狀態 |
|---|---|---|---|---|---|
| 7a | **不必按按鈕也知道吃了哪家**：決定前的「跳過 3 天」文案改掉（P0-B） | 已驗證 | ✅ **當天最後一次轉 = 午餐**，以「每個人」為單位（配合 7b） | A1、A2 | 📋 待開工 |
| 7b | **今日卡片**：輪盤頁顯示今天吃哪裡、誰決定的（P0-A） | 已驗證 | ✅ **可以多個（分團吃）**，不阻擋再轉 | A3 | 📋 |
| 7c | 「Use my location」建立的輪盤沒有步行時間（P0-C） | 已驗證 | ✅ **建完問一次「存成辦公室？」** | B1 | 📋 |
