...
function WheelActionsMenu({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="glass border-border/50 min-w-44"
      >
...
export default function WheelSelector({
    ...
    onSuccess: data => {
      ...
      if (addStarterPack) {
        importStarterPack.mutate(
          ...
          {
            onSuccess: () =>
              utils.restaurants.list.invalidate({ wheelId: data.id }),
          }
        );
      }
      if (data.inviteToken) {
  ...
  const inviteUrl = showInvite
    ? `${window.location.origin}/join/${showInvite.token}`
    : "";
   ...
   *  one thing. */
  const renderRow = (
    wheel: NonNullable<typeof wheels>[number],
    variant: "rail" | "sheet"
  ) => {
      ...
      <div
        ...
        style={{
          background: isSelected
            ...
            : "transparent",
          border: isSelected
            ? "1px solid oklch(from var(--brand) l c h / 0.3)"
          ...
          <WheelActionsMenu
            ...
            onDelete={() => {
              if (confirm(`Delete "${wheel.name}"?`))
                deleteWheel.mutate({ id: wheel.id });
            }}
          />
        </div>
      </div>
    ...
    <>
      ...
      <aside className="hidden md:flex w-56 flex-col gap-1 m-2 p-2 rounded-2xl glass-nav overflow-y-auto flex-shrink-0">
        <div className="px-2 pt-1 pb-2">
          <span
            className="text-xs font-semibold text-muted-foreground tracking-widest"
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="glass border-border/50 max-w-sm">
          ...
          <div className="flex flex-col gap-4 pt-2">
            <Input
              ...
              onKeyDown={e =>
                ...
                createWheel.mutate({
                  ...
                  isShared,
                  isPublic,
                  exclusionDays: parseInt(exclusionDays),
                  fairnessMode,
                  rotateCuisines,
                })
              }
            ...
            {rotateCuisines && (
              <p className="-mt-2 text-xs text-muted-foreground">
                Spins lean away from a cuisine you just had toward neglected
                ones.
              </p>
            )}
            <div className="flex items-center justify-between">
              <Label className="text-sm text-muted-foreground">
                Add starter restaurants
              </Label>
              <Switch
                checked={addStarterPack}
                onCheckedChange={setAddStarterPack}
              />
            </div>
            <ErrorChip
              error={createError}
        ...
        <DialogContent className="glass border-border/50 max-w-sm">
          ...
          <div className="flex flex-col gap-4 pt-2">
            <p className="text-sm text-muted-foreground">
              ...
              <strong className="text-foreground">{showInvite?.name}</strong>:
            </p>
          ...
          {editWheel && (
            <div className="flex flex-col gap-4 pt-2">
              <Input
                ...
                onChange={e =>
                  setEditWheel({ ...editWheel, name: e.target.value })
                }
                className="bg-secondary/50 border-border/50"
              />
              <div className="flex items-center justify-between">
                <Label className="text-sm text-muted-foreground">
                  Public (anyone with link can view &amp; spin)
                </Label>
                <Switch
                  checked={editWheel.isPublic}
                  onCheckedChange={v =>
                    setEditWheel({ ...editWheel, isPublic: v })
                  }
                />
              </div>
              <div className="flex items-center justify-between">
                ...
                <Select
                  value={String(editWheel.exclusionDays)}
                  onValueChange={v =>
                    setEditWheel({ ...editWheel, exclusionDays: parseInt(v) })
                  }
                >
                  <SelectTrigger
                    size="sm"
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={v =>
                    setEditWheel({ ...editWheel, fairnessMode: v })
                  }
                />
              </div>
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={v =>
                    setEditWheel({ ...editWheel, rotateCuisines: v })
                  }
                />
              </div>
              <ErrorChip
        ...
        <DialogContent className="glass border-border/50 max-w-md">
          ...
          <div className="flex flex-col gap-3 pt-2">
            <p className="text-xs text-muted-foreground">
              Paste a wheel export, or load a <code>.json</code> file. It's
              added as a new wheel you own.
            </p>
            <label
